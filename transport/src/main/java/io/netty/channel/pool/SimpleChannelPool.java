...
public class SimpleChannelPool implements ChannelPool {
        ...
        if (future.isSuccess()) {
            if (future.getNow() == Boolean.TRUE) {
                try {
                    ...
                    closeAndFlail(ch, cause, promise);
                }
            } else {
                closeChannel(ch);
                acquire(promise);
    ...
    public Future<Void> release(final Channel channel, final Promise<Void> promise) {
        ...
        try {
            ...
            closeAndFlail(channel, cause, promise);
        }
        return promise;
    }
        ...
        if (channel.attr(POOL_KEY).getAndSet(null) != this) {
            ...
            try {
                if (offerChannel(channel)) {
                    ...
                    closeAndFlail(channel, FULL_EXCEPTION, promise);
                }
            } catch (Throwable cause) {
                closeAndFlail(channel, cause, promise);
            }
    ...
    private static void closeAndFlail(Channel channel, Throwable cause, Promise<?> promise) {
